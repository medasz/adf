...
knownxss.encode = {};
knownxss.encode._escape = function(){_g('ende_textarea').value=escape(_g('ende_textarea').value);}
knownxss.encode._unescape = function(){_g('ende_textarea').value=unescape(_g('ende_textarea').value);}
knownxss.encode._encodeURI = function(){_g('ende_textarea').value=encodeURI(_g('ende_textarea').value);}
knownxss.encode._decodeURI = function(){_g('ende_textarea').value=decodeURI(_g('ende_textarea').value);}
knownxss.encode._encodeURIComponent = function(){_g('ende_textarea').value=encodeURIComponent(_g('ende_textarea').value);}
knownxss.encode._decodeURIComponent = function(){_g('ende_textarea').value=decodeURIComponent(_g('ende_textarea').value);}
knownxss.encode.en = function(x){
	...
	if(x==10){
		for(i=0;i<txt.length;i++){
			...
			else if(_g('dec_1').checked) {if(i<txt.length-1)_a+= txt.charCodeAt(i)+",";else{_a+= txt.charCodeAt(i)+",";_a="cos:expression(eval(String.fromCharCode(105,102,40,33,119,105,110,100,111,119,46,120,41,123,"+_a+"59,119,105,110,100,111,119,46,120,61,49,59,125)))";}}
...
knownxss.encode.copy_ok = function(){_g('ende_textarea').style.background='#DDDDDD';setTimeout("_g('ende_textarea').style.background='#FFFFFF'",700);}
...
knownxss.encode.base64Code = function(i){
	var txt=_g('ende_textarea').value;
	if (i == 1)
		_g('ende_textarea').value=knownxss.encode.base64encode(knownxss.encode.utf16to8(txt));
	if (i == 2)
		_g('ende_textarea').value=knownxss.encode.utf8to16(knownxss.encode.base64decode(txt));
}
...
knownxss.encode.utf7_encode = function() {
	_g('ende_textarea').value = utf7_encode(_g('ende_textarea').value);
};
knownxss.encode.utf7_decode = function() {
